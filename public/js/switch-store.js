(function(){
  ...
  const initial=()=>({version:3,ownerUserId:null,ownerTenantId:null,createdAt:now(),updatedAt:now(),retention:{active:'indefinite',expiredDays:90},transactions:[],batches:[],artifacts:[],events:[],atmMessages:[],lastAtmReconciliation:null,constructorPractices:[],isoMessages:[]});
  ...
  const ready=hydrate();
  function addTransaction(tx){const db=read();const routedAt=tx.routedAt||tx.createdAt||now();const record={id:tx.id||crypto.randomUUID?.()||('tx-'+Date.now()),createdAt:tx.createdAt||routedAt,channel:tx.channel||'POS',network:tx.network||'UNKNOWN',type:tx.type||'purchase',amountCents:Number(tx.amountCents||0),currency:tx.currency||'ARS',status:tx.status||'APPROVED',responseCode:tx.responseCode||'00',stan:tx.stan||'',rrn:tx.rrn||'',auth:tx.auth||'',batch:Number(tx.batch||1),closed:Boolean(tx.closed),clearingStatus:tx.clearingStatus||'PENDING',onlineStatus:tx.onlineStatus||'ROUTED',routedAt,mtiRequest:tx.mtiRequest||'0200',mtiResponse:tx.mtiResponse||'0210',panLast4:tx.panLast4||'',cardId:tx.cardId||'',product:tx.product||'',raw:tx.raw||null,requestFields:tx.requestFields||null,responseFields:tx.responseFields||null,operationLabel:tx.operationLabel||'',dateTime:tx.dateTime||''};
    const idx=db.transactions.findIndex(x=>x.id===record.id);if(idx>=0)db.transactions[idx]={...db.transactions[idx],...record};else db.transactions.unshift(record);db.events.unshift({at:routedAt,kind:'ONLINE_ROUTING',text:`${record.channel} ${record.network} ${record.type} enrutada online al Switch adquiriente`});return write(db)}
  ...
  function reconcileATM(payload={}){const db=read();const atmId=payload.atmId||'ATM00001';const candidates=db.transactions.filter(t=>t.channel==='ATM'&&!t.closed);let readyCount=0,excludedCount=0,amexCount=0;
    ...
    db.events.unshift({at:now(),kind:'ATM_RECONCILIATION',text:`${atmId} conciliado: totales confirmados para ${candidates.length} operaciones ya enrutadas online`});
    write(db);return rec
  }
  ...
  function addConstructorPractice(practice){const db=read(),record={id:practice.id||`constructor-${Date.now()}-${Math.random().toString(16).slice(2,7)}`,createdAt:practice.createdAt||now(),mode:practice.mode||'constructor',network:practice.network||'auto',mti:String(practice.mti||''),bitmap:String(practice.bitmap||''),annotated:String(practice.annotated||''),raw:String(practice.raw||''),valid:practice.valid!==false,missing:Array.isArray(practice.missing)?practice.missing:[]};db.constructorPractices=Array.isArray(db.constructorPractices)?db.constructorPractices:[];db.isoMessages=Array.isArray(db.isoMessages)?db.isoMessages:[];db.constructorPractices.unshift(record);db.constructorPractices=db.constructorPractices.slice(0,50);return write(db)}

  // Parser Guiado: intentos del alumno. Se guardan dentro de constructorPractices (mode 'guided-parser') para no requerir cambios en la API.
  function addGuidedParserAttempt(rec){const db=read();db.constructorPractices=Array.isArray(db.constructorPractices)?db.constructorPractices:[];db.isoMessages=Array.isArray(db.isoMessages)?db.isoMessages:[];const i=db.constructorPractices.findIndex(x=>x.id===rec.id);const record={...rec,mode:'guided-parser'};if(i>=0)db.constructorPractices[i]=record;else db.constructorPractices.unshift(record);const guided=db.constructorPractices.filter(x=>x.mode==='guided-parser').slice(0,40);const others=db.constructorPractices.filter(x=>x.mode!=='guided-parser').slice(0,50);db.constructorPractices=[...guided,...others].sort((a,b)=>String(b.createdAt).localeCompare(String(a.createdAt)));return write(db)}

  function addIsoMessage(msg){const db=read();db.isoMessages=Array.isArray(db.isoMessages)?db.isoMessages:[];const record={id:msg.id||`iso-${Date.now()}-${Math.random().toString(16).slice(2,7)}`,transactionId:msg.transactionId||'',createdAt:msg.createdAt||now(),channel:msg.channel||'UNKNOWN',network:msg.network||'UNKNOWN',direction:msg.direction||'REQUEST',operation:msg.operation||'',mti:String(msg.mti||''),bitmap:String(msg.bitmap||''),fields:msg.fields||{},raw:String(msg.raw||''),amountCents:Number(msg.amountCents||0),currency:msg.currency||'',responseCode:msg.responseCode||'',status:msg.status||'',panLast4:msg.panLast4||''};db.isoMessages.unshift(record);db.isoMessages=db.isoMessages.slice(0,500);return write(db)}
  ...
  window.OSCSwitchStore={read,write,addTransaction,closeBatch,reconcileATM,generateClearing,setAtmMessages,setLastAtmReconciliation,addConstructorPractice,addGuidedParserAttempt,addIsoMessage,flush,reset,ready,key:KEY};
})();
